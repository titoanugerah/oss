<div class="panel-header bg-danger-gradient">
  <div class="page-inner py-5">
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
      <div>
        <h2 class="text-white pb-2 fw-bold">Edit Kegiatan</h2>
      </div>
                ...
                <div class="form-group col-md-4">
                  ...
                  <select class="form-control" name="start_id">
                    <?php foreach ($schedule as $item): ?>
                      <option class="form-control" value="<?php echo $item->id ?>" <?php if($item->id == $history->start_id){ echo 'selected';} ?>><?php echo $item->day.' jam '.$item->start.' s/d '.$item->end; ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="form-group col-md-4">
                  ...
                  <select class="form-control" name="end_id">
                    <?php foreach ($schedule as $item): ?>
                      <option class="form-control" value="<?php echo $item->id ?>" <?php if($item->id == $history->end_id){ echo 'selected';} ?>><?php echo $item->day.' jam '.$item->start.' s/d '.$item->end; ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="form-group col-md-4">
                  <label>Nama Kegiatan</label>
                  <input type="text" name="name" value="<?php echo $history->name; ?>" class="form-control">
                </div>
                <div class="form-group col-md-4">
                  <label>Penyelenggara</label>
                  <input type="text" name="remark" value="<?php echo $history->remark; ?>" class="form-control">
                </div>
                <div class="form-group col-md-4">
                  <label>Tanggal</label>
